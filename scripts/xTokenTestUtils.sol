...
/**
 * @title TestUtils
 * @notice Base contract with common test functions for xToken testing
 * @dev Subclasses must implement token-specific configuration
 */
abstract contract TestUtils is Script {
    Proxy public proxy;
    xToken public token;
    ...
    // Token-specific configuration (to be provided by subclasses)
    function getTokenName() internal pure virtual returns (string memory);
    function getTokenSymbol() internal pure virtual returns (string memory);
    function getTokenDecimals() internal pure virtual returns (uint8);
    function getMaxSupply() internal pure virtual returns (uint256);
    ...
    function testReadFunctions() internal view {
        ...
        require(keccak256(bytes(name)) == keccak256(bytes(getTokenName())), "Name mismatch");
        
        // Test symbol()
        string memory symbol = token.symbol();
        console.log("symbol():", symbol);
        require(keccak256(bytes(symbol)) == keccak256(bytes(getTokenSymbol())), "Symbol mismatch");
        
        // Test decimals()
        uint8 decimals = token.decimals();
        console.log("decimals():", decimals);
        require(decimals == getTokenDecimals(), "Decimals mismatch");
        ...
        require(maxSupply == getMaxSupply(), "Max supply mismatch");
    ...
    function testMinting() internal {
        ...
        uint256 mintAmount = 1000 * 10 ** getTokenDecimals();
    ...
    function testTransfers() internal {
        ...
        uint256 transferAmount = 100 * 10 ** getTokenDecimals();
    ...
    function testApprovals() internal {
        ...
        uint256 approveAmount = 50 * 10 ** getTokenDecimals();
        ...
        uint256 transferAmount = 30 * 10 ** getTokenDecimals();
    ...
    function testBurning() internal {
        ...
        uint256 burnAmount = 50 * 10 ** getTokenDecimals();
        ...
        // Test burn() - admin has MINTER_ROLE
        vm.startBroadcast(adminPK);
