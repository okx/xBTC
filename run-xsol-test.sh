...
# Script to deploy and test xSOL token on local Anvil testnet
# This script will:
# 1. Start a local Anvil testnet (if not already running)
# 2. Deploy the xSOL contracts
# 3. Run comprehensive tests on all functions

set -e

echo "==================================="
echo "xSOL Deployment and Testing Script"
echo "==================================="
echo ""
...
echo ""
echo -e "${BLUE}Running xSOL test script...${NC}"
forge script scripts/Test.xSOL.testnet.s.sol:TestXSOL \
    --rpc-url $RPC_URL \
    --broadcast
